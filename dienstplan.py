...
class Dienstplan:
	...
	def _extractShifts(self, useSecondLine=False):
		...
		for lineno, line in enumerate(lines):
			...
			if not len(tokens[1:]) == 0 and len(tokens[1:]) >= len(curShifts):
				#extract shift information only if information is present. the lowest line with more information is probably the right one
				curShifts = tokens[1:]
